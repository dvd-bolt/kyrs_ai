...
def test_ras_trial_balance_accepts_simple_and_compound_postings() -> None:
    ...
    assert next(line for line in balance.lines if line.account == "20").debit_turnover == Decimal("60.00")
...
def test_financial_oracles_for_analysis_investment_and_credit() -> None:
    horizontal = horizontal_analysis({"revenue": "120"}, {"revenue": "100"}, spec=_spec("horizontal"))
    ...
    vertical = vertical_analysis(
        {"assets": "200", "cash": "50", "inventory": "30"}, total_key="assets", spec=_spec("vertical")
    )
    assert vertical.values["cash_share_percent"] == Decimal("25.00")
    ratios = financial_ratios(
        {
            "current_assets": "300", "current_liabilities": "150", "inventory": "60", "cash": "30",
            "equity": "400", "total_assets": "800", "total_liabilities": "400", "net_profit": "80",
            "revenue": "1000", "average_assets": "750", "average_equity": "350", "cost_of_sales": "600",
            "average_inventory": "75", "average_receivables": "125",
        }, spec=_spec("ratios")
    )
    assert ratios.values["current_ratio"] == Decimal("2.00")
    assert ratios.values["return_on_sales_percent"] == Decimal("8.00")
    breakeven = break_even(fixed_costs="100", price="10", variable_cost_per_unit="6", spec=_spec("break_even"))
    assert breakeven.values["break_even_units"] == Decimal("25.00")
    investment = investment_metrics(["-100", "60", "60"], discount_rate="0.1", spec=_spec("investment"))
    assert investment.values["npv"] == Decimal("4.13")
    assert investment.values["irr_percent"] == Decimal("13.07")
    schedule = loan_schedule(principal="1200", annual_rate="0.12", periods=12, start_date=date(2025, 1, 31), method=LoanMethod.ANNUITY)
...
@pytest.mark.parametrize(
    "call",
    [
        lambda: break_even(fixed_costs="1", price="1", variable_cost_per_unit="1", spec=_spec("break_even")),
        lambda: vertical_analysis({"assets": "0", "cash": "1"}, total_key="assets", spec=_spec("vertical")),
        lambda: investment_metrics(["-100", "20"], discount_rate="0.1", spec=_spec("investment")),
    ],
)
def test_financial_calculations_fail_closed_on_zero_or_unpaid_values(call: object) -> None:
