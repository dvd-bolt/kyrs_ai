...
if (-not $launcher) {
    throw "Python Launcher (py.exe) is required. Install 64-bit Python 3.12 first."
}

& $launcher.Source -3.12 -c "import sys; assert sys.version_info[:2] == (3, 12)"
if ($LASTEXITCODE -ne 0) {
    throw "64-bit Python 3.12 is required for the Windows build."
}

$environmentDir = Join-Path $repositoryRoot ".venv-build"
$python = Join-Path $environmentDir "Scripts\python.exe"
if (-not (Test-Path -LiteralPath $python -PathType Leaf)) {
    & $launcher.Source -3.12 -m venv $environmentDir
